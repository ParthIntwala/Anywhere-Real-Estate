...
void main() {
  test("Let's test data fetched for iOS Application", () async {
    //declare
    FetchData data = FetchData();

    //run
    await data.fetchCharacters("simpsons+characters");
    List<Character> fetchedCharacters = data.characters;

    //verify
    expect(fetchedCharacters.length, characters.length);
    expect(fetchedCharacters.reversed, isNot(60));
    for (int i = 0; i < fetchedCharacters.length; i++) {
      expect(fetchedCharacters[i].name, characters[i].name);
      expect(fetchedCharacters[i].description, characters[i].description);
      expect(fetchedCharacters[i].url, characters[i].url);
    }
  });

  test("Let's test data fetched from incorrect query string", () async {
    //declare
    FetchData data = FetchData();

    //run
    await data.fetchCharacters("simpson+characters");
    List<Character> fetchedCharacters = data.characters;

    //verify
    expect(fetchedCharacters.length, emptyCharacters.length);
    expect(fetchedCharacters.reversed, isNot(60));
    for (int i = 0; i < fetchedCharacters.length; i++) {
      expect(fetchedCharacters[i].name, emptyCharacters[i].name);
      expect(fetchedCharacters[i].description, emptyCharacters[i].description);
      expect(fetchedCharacters[i].url, emptyCharacters[i].url);
    }
  });
}

List<Character> emptyCharacters = [];
